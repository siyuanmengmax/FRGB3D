"""FRGB3D: Fast Reliability-Weighted Gaussian Background Modeling.

Implements the core FRGB3D algorithm described in the paper:
  1. Physics-based point reliability estimation from range and reflectivity.
  2. Reliability-weighted adaptive Gaussian background modeling.
  3. Reliability-weighted adaptive foreground detection.
"""
import numpy as np
...
def calculate_reliability(ranges, reflectivity):
    ...
    anchored at the 10% and 90% calibration points (see paper Appendix,
    "Reflectivity Coefficient Derivation").
    ...
    # Calibration coefficients at the 10% and 90% reflectivity anchor points.
    ...
    for i in prange(len(ranges)):
        ...
        # Quadratic range-dependent precision estimate (cm), then converted
        # to a reliability score.
...
def update_background_model(mean, std, weight, mean_reflectivity, counts,
    ...
    for i in prange(len(indices)):
        ...
        counts[idx] += 1
        if weight[idx] == 0:
            # First observation at this position.
            mean[idx] = r
            weight[idx] = rel
...
def detect_foreground_points(indices, ranges, reliability, reflectivity, mean, std, mean_reflectivity,
    ...
    for i in prange(len(indices)):
        ...
        if counts[idx] >= min_count and mean[idx] > 0:
            ...
            # Adaptive threshold: tau_i = tau_0 + sigma_i (paper Eq. for Algorithm 2).
...
def build_background(pcd_files, min_range=0.5, max_range=200,
    ...
    """Build the FRGB3D reliability-weighted background model from a sequence of frames."""
    ...
    for i in tqdm(range(0, len(pcd_files), interval), desc="Building FRGB3D background model"):
    ...
    print(f"FRGB3D background model built: {np.sum(valid_points)}/{total_points} valid points "
          f"({np.sum(valid_points) / total_points:.2%})")
    return bg_model


def detect_foreground(pcd_array, bg_model, base_threshold=1, base_threshold_refl=0, min_count=10, min_range=0.5,
                       max_range=200, range_factor=1,
                       visualize=False, bg_model_columns=['mean', 'std', 'mean_reflectivity', 'counts'],
                       columns=['x', 'y', 'z', 'range', 'reflectivity', 'near_ir', 'intensity', 'flags']):
    """Detect foreground points in a frame given a fitted FRGB3D background model."""
