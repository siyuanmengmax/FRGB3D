"""Object-level Average Precision (AP) evaluation for the real-world experiment.

Compares detected bounding boxes (from run_realworld_detection.py) against
manually annotated ground truth (Segments.ai export format) using rotated
2D IoU matching, and reports AP@<iou_threshold> per category plus the
mean AP across categories (Small Vehicle, Large Vehicle, Pedestrian) --
matching the paper's Table 4 (AP@0.1 / AP@0.3 / AP@0.5).

Example:
    python scripts/evaluate_detection_ap.py \\
        --groundtruth_json bin/lowell_1000_label.json \\
        --detection_csv results/realworld_detection/bboxes.csv \\
        --iou_threshold 0.3
"""
import os
import sys
...
def load_detections(csv_file):
    """Load detected boxes from run_realworld_detection.py's output CSV.
...
def evaluate_detections(gt_data, det_data, iou_threshold=0.1):
    ...
    for frame_id in tqdm(frame_ids):
        ...
        matched_gt_indices = {}  # Per-category set of already-matched ground-truth indices.
...
def main(args):
    gt_data = load_groundtruth(args.groundtruth_json)
    det_data = load_detections(args.detection_csv)

    ap_per_class, mAP = evaluate_detections(gt_data, det_data, iou_threshold=args.iou_threshold)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Object-level AP evaluation for the real-world experiment")
    parser.add_argument("--pcd_folder", default="bin/merge_1000", help="Folder of point cloud frames (only used for --visualize)")
    parser.add_argument("--detection_csv", default="results/realworld_detection/bboxes.csv", help="Detection results CSV")
