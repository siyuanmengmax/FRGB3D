"""Dual-LiDAR real-world preprocessing: FRGB3D background modeling + fusion.

For each pair of time-aligned frames from two roadside LiDAR sensors, this
builds an FRGB3D reliability-weighted background model per sensor, runs
foreground detection, transforms both point clouds into a common ground/
registration frame, and fuses them into a single point cloud with
per-point foreground mask, reliability, and source-sensor id columns.

The fused output feeds the real-world detection/clustering pipeline
(see run_realworld_detection.py) and the object-level AP evaluation
(see evaluate_detection_ap.py).

Requires the registration matrices produced by registration.py and the
time-aligned frame lists produced by data_preparation/time_alignment.py.
"""
import os
import sys
import argparse
...
from src.frgb3d.background_model import build_background, detect_foreground
...
def main(args):
    ...
    source_bg_model_path = os.path.join(args.output_folder_path, 'one_bg_model.npy')
    target_bg_model_path = os.path.join(args.output_folder_path, 'two_bg_model.npy')
    ...
    for i, (source_pcd_file, target_pcd_file) in tqdm(enumerate(zip(source_pcd_files, target_pcd_files))):
        ...
        merge_pcd_name = os.path.basename(source_pcd_file).replace("one", "lowell_fuse")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Dual-LiDAR FRGB3D background removal and fusion")
